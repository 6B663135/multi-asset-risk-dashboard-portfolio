#code taken from EDHEC Course which explains portfolio optimization using Python
...
def var_historic(r, level=5):
    ...
    Example:
    level=5 returns the number such that 5% of the returns
    fall below that number and 95% are above.
    """
...
def var_gaussian(r, level=5, modified=False):
    ...
    """
    # Compute the Z-score assuming returns are Gaussian
...
def gmw(cov):
    """
    Returns weight of the GMV portfolio given the covariance matrix
    """
    n = cov.shape[0]
...
def plot_ef(n_points, er, cov, show_cml=False, style='.-', riskfree_rate=0, show_ew=False, show_gmv=False):
    ...
    if show_gmv:
        ...
        ax.plot([vol_gmw],[r_gmw], color="midnightblue",marker="o")
...
def show_cppi(n_scenarios=50, mu=0.07, sigma=0.15, m=3, floor=0, riskfree_rate=0.03, steps_per_year=12, y_max=100):
    ...
    e_shortfall = np.dot(
        ...
        failure_mask
    ) / n_failures if n_failures > 0 else 0.0
...
def discount(t,r):
    print("discount r type:", type(r))
    print("discount r shape:", getattr(r, "shape", None))
    discounts = pd.DataFrame([(r+1)**-i for i in t])
    discounts.index = t
    return discounts

def pv(flows,r):
    # compute PV of a sequence of cash flows
    dates = flows.index
    discounts = discount(dates, r)
    return discounts.multiply(flows, axis='rows').sum()

def funding_ratio(asset, liabilities, r):
    # computes funding ratio given the parameters
    return pv(asset,r)/pv(liabilities,r)
...
def ann_to_inst(r):
    """
    annualized to short rate
    """
    return np.log1p(r)

def cir(n_years = 10, n_scenarios = 1, a = 0.05, b = 0.03, sigma = 0.05, steps_per_year = 12, r_0 = None):
    """ implements the CIR model for int. rates """
...
def show_cir(r_0=0.03, a=0.5, b=0.03, sigma=0.05, n_scenarios=5):
    print(r_0, a, b, sigma, n_scenarios)
    cir(r_0=r_0, a=a, b=b, sigma=sigma, n_scenarios=n_scenarios).plot(legend=False, figsize=(12,5))
    plt.show()

def cir2(n_years=10, n_scenarios=1, a=0.05, b=0.03, sigma=0.05, steps_per_year=12, r_0=None):
    """
    Generate random interest rate evolution over time using the CIR model.
    b and r_0 are assumed to be annualized rates, not the short rate,
...
'''def macaulay_duration(flows, discount_rate):
    ...
    #return np.average(flows.index, weights=weights.squeeze())
    return (flows.index * weights.iloc[:, 0]).sum()
'''
def macaulay_duration(flows, discount_rate):
    discounts = pd.Series(
        [(1 + discount_rate) ** -i for i in flows.index],
        index=flows.index
    )
...
def bt_mix(r1, r2, allocator, **kwargs):
    ...
    and produces an allocation to the first portfolio as a T x 1 DataFrame.
...
def terminal_stats(rets, floor=0.8, cap=np.inf, name="Stats"):
    ...
    Returns a 1-column DataFrame of summary stats indexed by stat name.
    """
